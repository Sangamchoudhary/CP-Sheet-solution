...
void solve(){
    ll n; cin>>n;
    vll arr(n); inp(arr);
    ll mx = findMax(arr), mn = findMin(arr);
    if(mx == mn) {print(0ll); return;}
    ll pos1,pos2;
    rep(i,0,n)
        if(arr[i] == mx) {pos1 = i; break;}
    rev(i,n-1,0)
        if(arr[i] == mn) {pos2 = i; break;}
    if(pos1 < pos2) print(pos1 + n-pos2-1);
    else print(pos1 + n-pos2-2);
}
int main() {
